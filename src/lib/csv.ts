...
export function buildYearlyResultsCsv(rows: YearlyResult[]): string {
  ...
  const lines = rows.map((row) =>
    [
      ...
      row.scpiTaxableIncomeInFrance.toFixed(2),
      row.scpiTaxableIncomeAbroad.toFixed(2),
      row.scpiTaxesPaidInFrance.toFixed(2),
      row.scpiTaxesPaidAbroad.toFixed(2),
